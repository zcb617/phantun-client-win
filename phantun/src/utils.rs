...
mod windows {
    ...
    pub fn new_udp_reuseport(local_addr: SocketAddr) -> UdpSocket {
        // Windows does not support SO_REUSEPORT, just bind normally
        let std_sock = std::net::UdpSocket::bind(local_addr).unwrap();
        std_sock.set_nonblocking(true).unwrap();
        UdpSocket::from_std(std_sock).unwrap()
    }
    ...
    pub async fn udp_recv_pktinfo(
        ...
        let (size, addr) = sock.recv_from(buf).await?;
        // On Windows we don't have pktinfo, just use the bound address as dst
        let dst = sock.local_addr()?.ip();
        Ok((size, addr, dst))
    }

    pub fn assign_ipv6_address(_device_name: &str, _local: Ipv6Addr, _peer: Ipv6Addr) {
        // IPv6 address assignment on Windows is not supported in this minimal port
        // Windows users should use --ipv4-only
    }
}
