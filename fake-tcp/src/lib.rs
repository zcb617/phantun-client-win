...
impl Socket {
    ...
    async fn connect(&mut self) -> Option<()> {
        for retry in 0..RETRIES {
            match self.state {
                State::Idle => {
                    ...
                    info!("[connect] Retry {}/{}: Sending SYN from {} to {} (seq={})",
                          retry + 1, RETRIES, self.local_addr, self.remote_addr,
                          self.seq.load(Ordering::Relaxed));
                ...
                State::SynSent => {
                    ...
                    match time::timeout(TIMEOUT, self.incoming.recv_async()).await {
                        Ok(buf) => {
                            ...
                            info!("[connect] Received packet from {} to {} flags=0x{:02x} seq={} ack={}",
                                  ip_packet.get_source(), ip_packet.get_destination(),
                                  flags, tcp_packet.get_sequence(), tcp_packet.get_acknowledgement());
                            ...
                            {
                                info!("[connect] Got valid SYN+ACK (ack={} matches expected {}), sending ACK",
                                      tcp_packet.get_acknowledgement(), expected_ack);
                                self.seq.fetch_add(1, Ordering::Relaxed);
                                self.ack
                                ...
                                info!("[connect] Unexpected packet: flags=0x{:02x} (expected SYN+ACK=0x{:02x}), ack={} (expected {})",
        ...
        error!("[connect] All {} retries exhausted, fake TCP connection failed", RETRIES);
        None
    }
}
...
impl Stack {
    ...
    pub fn new(tun: Vec<Arc<dyn TunDevice>>, local_ip: Ipv4Addr, local_ip6: Option<Ipv6Addr>) -> Stack {
